      ...
      ddb.scan(params,function(err,result){
        if(err){
            ...
            result.forEach(archivo => {
                ...
                var bucket = `concurso-${archivo.idconcurso}`;
                var rutas3 = `${URLS3}/${bucket}//inicial/${archivo.voz_inicial}`;
                var proc = new ffmpeg({
                    source: rutas3,
                    nolog: true
                });
                console.log("Ruta S3: ", rutas3);
                ...
                let start = new Date();
                let filename = "/convertida/" + voz + ".mp3";
                date.format(start, 's');
                ...
                proc.setFfmpegPath(path)
                    ...
                    .on('end', () => {
                        ...
                        fs.readFile(RUTA_GESTOR_ARCHIVOS + archivo.idconcurso + '//convertida//' + voz + '.mp3', (err,data)=>{
                            if(err){
                                console.error(err);
                            }
                            s3.guardarArchivoEnS3(bucket, filename, data);
                        });
